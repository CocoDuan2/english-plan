...
### 结构（5个模块，固定）
0. **开始页** — 佩琪 + "开始玩"按钮（解锁 AudioContext）
1. **学新词** — 点击灰色卡片揭示 Twemoji 物品 + 发音
2. **主题游戏** — 跟主题相关的互动（每课不同）
3. **记忆翻牌** — 配对游戏
4. **Done 页** — 佩琪全家庆祝 + "All Done" + Play Again
...
- 顶栏中文提示样式（.tz）：渐变背景（#ff6b9d→#ffa07a）+ 白色文字 + 圆角20px + 柔和阴影，禁止灰底纯文本，禁止左侧图标（如💡灯泡）。
